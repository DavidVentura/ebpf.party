...
However, notice that it's not a regular pointer, the `__data_loc` prefix means it's dynamically sized.
...
Basically, `filename` actually contains only 4 bytes, which could be better understood as
