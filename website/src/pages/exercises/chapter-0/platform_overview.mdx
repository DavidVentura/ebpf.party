...
- <kbd>Ctrl</kbd>+<kbd>Click</kbd> on type definitions (`my_struct`, `trace_event_raw_sched_process_exec`) will show a little popup

### Debugging utilities

There are multiple `DEBUG_` macros to send data to the front end for easy visualization.
...
Use the `LEN` variant when you can't guarantee your buffer will be zero-initialized. Trailing null bytes are trimmed from answers, but trailing garbage is not.
