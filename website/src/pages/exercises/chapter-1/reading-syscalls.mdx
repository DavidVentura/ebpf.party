...
This `ctx->args[0]` pointer points to memory owned by the calling process (e.g., `bash`), which is **user space** memory.
...
1.  Retrieve the filename pointer from `ctx->args[0]`.
